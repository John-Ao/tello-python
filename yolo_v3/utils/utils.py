...
def scale_coords(img1_shape, coords, img0_shape):
    ...
    # print ((img1_shape,coords,img0_shape) )
    gain = float(max(img1_shape)) / float(max(img0_shape))  # gain  = old / new
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    if not (line_thickness == None):
        tl = line_thickness 
    else:
        tl = int(round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1 ) # line thickness
